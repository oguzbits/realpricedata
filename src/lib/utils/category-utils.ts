 ...
 */
export function filterProducts(products: Product[], filters: FilterState, categorySlug: string, unitLabel: string): Product[] {
  ...
  if (filters.formFactor && filters.formFactor.length > 0) {
    filtered = filtered.filter(p => filters.formFactor.includes(p.formFactor));
  }

  // 5. Capacity Filters
  if (filters.minCapacity !== null) {
    const minValReal = unitLabel === 'TB' ? filters.minCapacity * 1000 : filters.minCapacity;
    filtered = filtered.filter(p => (p.normalizedCapacity ?? p.capacity) >= minValReal);
  }

  if (filters.maxCapacity !== null) {
    const maxValReal = unitLabel === 'TB' ? filters.maxCapacity * 1000 : filters.maxCapacity;
    filtered = filtered.filter(p => (p.normalizedCapacity ?? p.capacity) <= maxValReal);
  }
 ...
 */
export function sortProducts(products: Product[], sortBy: string, sortOrder: string): Product[] {
