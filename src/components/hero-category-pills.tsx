...
export function HeroCategoryPills({ country }: { country: CountryCode }) {
  return (
    <div className={cn("flex flex-wrap gap-2")}>
      <span className={cn("text-muted-foreground self-center text-base")}>
        Popular:
      </span>
      {categories.map((category) => {
        ...
        return (
          <Link
            ...
            className={cn(
              "group border-primary/20 bg-secondary hover:bg-secondary/80 flex items-center gap-2 rounded-full border px-4 py-2 no-underline shadow-sm transition-all",
            )}
            prefetch={true}
          >
            <Icon className={cn("text-primary h-3.5 w-3.5")} />
            <span
              className={cn(
                "text-primary text-base font-bold transition-colors group-hover:underline",
              )}
            >
              {category.name}
            </span>
